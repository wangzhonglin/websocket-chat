$(document).ready(function(){
	...
	function checkCookies() {
		...
		if (data == null) {
			...
			alert(loginSessionId + userId + userName + userNickname);
		}
	} checkCookies();
	...
	function remove_session(userId) {
		$('.contact-list li#user-'+userId).addClass('offline').delay(1000).slideUp(800,function(){
			$(this).remove();
		});
	}

	function add_session(userId,userName,unreadMsgCount) {
		$('.contact-list li:first').delay(1000).slideDown(800,function(){
			var newSession = document.createElement('li');
			newSession.addClass('offline');
			newSession.setAttribute('id', 'user-'+userId);
			newSession.innerHTML = '<a href="#"><img alt="" src="../images/demo/av1.jpg" /> <span>' + userName + '</span></a><span class="msg-count badge badge-info-msg">' + unreadMsgCount + '</span>';
		});
	}
	add_session(123,'name',3);
});